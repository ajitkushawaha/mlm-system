...
async function getConnectionStats(userId: ObjectId, db: Db) {
  const user = await db.collection<User>("users").findOne({ _id: userId })
  if (!user) return null

  // Count total connection size recursively
  async function countConnection(nodeId: ObjectId, visited = new Set()): Promise<number> {
    ...
    if (node.leftChild) {
      count += await countConnection(node.leftChild, visited)
    }
    if (node.rightChild) {
      count += await countConnection(node.rightChild, visited)
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = await getConnectionStats(new ObjectId(decoded.userId), db)
